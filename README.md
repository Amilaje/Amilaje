...
**빅 데이터 분석기사** 필기 합격, ~*실기 준비중*~

---

## 💡 Projects

### 기상청 데이터 공모전 – 열수요 예측 모델 (입상)
- 공공 기상 데이터와 시계열 데이터를 기반으로 열수요 예측 모델 설계
- Fourier Series 기반 계절성 변수(sin, cos)와 기상 변수(온도, 습도 등) 활용
- LightGBM과 Stacking 모델을 통해 MAE 성능 개선 달성
- 시각화 및 변수 중요도 분석을 통한 인사이트 도출 및 보고서 작성
  
→ **내 역할**:팀장, 데이터 전처리 및 피처 엔지니어링, Stacking 모델 설계, 보고서 집필
...
→ **내 역할**: 팀장, MSA 아키텍처 설계, Kafka 이벤트 흐름 설계 및 AI 기능 전체 구현
...
→ **내 역할**: 팀장, LLM 파이프라인 설계 및 주요 Agent 로직 개발
...
→ **내 역할**: 서기, 데이터 분석, ML 모델 비교 실험 및 특징 엔지니어링
